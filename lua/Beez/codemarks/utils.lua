...
function M.path.find_git_dir()
  ...
  while current_path do
    ...
    local git_path = u.paths.Path:new(current_path, ".git")
    if git_path:exists() then
      M.cache.git_dirs[current_path] = git_path:absolute()
      return M.cache.git_dirs[current_path]
    end
    current_path = u.paths.Path:new(current_path):parent():absolute()
    if current_path == u.paths.Path.path.root(current_path) then
      break
    end
  end
...
function M.path.get_root_dir()
  local git_dir = u.root.find_git_dir()
  local root = nil
  if git_dir then
    root = u.paths.basename(u.paths.Path:new(git_dir):parent().filename)
  else
    root = "rootless"
  end
  return root
